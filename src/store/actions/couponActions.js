...
export const addCoupon = (coupon) => {
    return (dispatch, getState, { getFiresbase, getFirestore }) => {
        ...
        firestore.collection('coupons').doc(coupon.name).set({
            ...coupon,
            name: coupon.name,
...
export const updateCoupon = (coupon) => {
    return (dispatch, getState, { getFirebase, getFirestore }) => {
        const firestore = getFirestore();
        firestore.collection('coupons').doc(coupon.name).update({
            ...coupon,
            discount: coupon.discount,
        }).then(() => {
...
export const deleteCoupon = (coupon) => {
    return (dispatch, getState, { getFirebase, getFirestore }) => {
        const firestore = getFirestore();
        firestore.collection('coupons').doc(coupon.name).delete()
            .then(() => {
